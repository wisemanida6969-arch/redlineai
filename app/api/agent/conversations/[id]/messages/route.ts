...
const SEARCH_TOOL: Anthropic.Tool = {
  name: "search_standard_contract",
  description:
    "Search the real Korean government (MCST) standard-contract article database for a specific clause by field, contract type, and topic. Returns verbatim official article text — use this whenever the user asks what a standard contract says or requires about something, so you can quote the real text instead of relying on general knowledge. Returns no results if nothing matches; in that case say so rather than inventing an answer.",
  input_schema: {
    type: "object",
    properties: {
      category: {
        type: "string",
        description: "창작 분야 (optional), e.g. 미술, 웹툰, 공연예술, 영화, 공예. Leave empty to search all fields.",
      },
      contract_type: {
        type: "string",
        description: "구체적 계약 유형명 (optional), e.g. '웹툰 연재계약서', 'Screenplay'. Leave empty to search all types in the field.",
      },
      topic: {
        type: "string",
        description: "찾고 싶은 조항 주제, e.g. 저작권, 지체상금, 수정 횟수, 대금 지급, 계약 해지, 손해배상, 비밀유지, 분쟁 해결, 저작인격권, 소유권",
      },
    },
    required: ["topic"],
  },
...
**Standard-contract search tool**: You have a search_standard_contract tool over a real database of verbatim Korean government (MCST) standard-contract articles (5 fields, 35 contract types, 700+ articles). Use it whenever the user asks what a standard contract says, requires, or permits about a topic — quote the returned article text directly rather than answering from general knowledge. If the tool returns nothing, say plainly that you couldn't find a matching standard-contract provision — never invent article text.
...
**표준계약서 검색 도구**: 실제 문화체육관광부(MCST) 표준계약서 원문 조항 데이터베이스(5개 분야, 35개 계약 유형, 700개 이상 조항)를 검색하는 search_standard_contract 도구를 사용할 수 있습니다. 사용자가 표준계약서가 특정 주제에 대해 어떻게 규정하는지 물으면, 일반 지식으로 답하지 말고 이 도구로 찾은 실제 원문을 그대로 인용해 답변하세요. 검색 결과가 없으면 지어내지 말고 해당하는 표준계약서 조항을 찾지 못했다고 솔직히 답하세요.
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    /* ── Call Claude, looping through standard-contract search tool calls ── */
    let assistantText = "";
    for (let turn = 0; turn < 4; turn++) {
      ...
      const toolResults = await Promise.all(toolUseBlocks.map(async (b) => {
        ...
        const input = block.input as { category?: string; contract_type?: string; topic: string };
        const results = await searchStandardArticles(
          service,
          { category: input.category, contractType: input.contract_type, topic: input.topic },
          lang,
        );
        const content = results.length === 0
          ? (lang === "ko" ? "일치하는 표준계약서 조항을 찾지 못했습니다." : "No matching standard-contract article found.")
          : results.map((r) =>
              `[${r.categoryTitle} - ${r.typeTitle}] 제${r.articleNo}조(${r.articleTitle}): ${r.articleText}`
            ).join("\n\n");

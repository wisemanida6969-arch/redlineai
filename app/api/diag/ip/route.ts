...
/**
 * Diagnostic: reports this server's OUTBOUND IP(s) (to register at open.law.go.kr)
...
export async function GET() {
  // Outbound IP (the address law.go.kr sees when we call it)
  ...
  // Live 법제처 test (OC never exposed — only the response snippet)
  ...
  return NextResponse.json({
    ...
    hint: lawWorking
      ? "법제처 API 정상 작동 중입니다."
      : "위 outboundIp(들) 를 open.law.go.kr 서버 IP로 등록하세요. Static IP는 여러 번 호출해 모든 IP를 확인하세요.",
  });
}
